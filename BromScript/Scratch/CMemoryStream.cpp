...
CMemoryStream::CMemoryStream(void):strm_pubBuffer(NULL),strm_ulPosition(0),strm_ulSize(0),strm_ulUsed(0){
...
unsigned long CMemoryStream::Size()
{
  return strm_ulUsed;
}

unsigned long CMemoryStream::Location()
{
  return strm_ulPosition;
}

void CMemoryStream::Seek(unsigned long ulPos, int iOrigin)
{
  switch(iOrigin) {
...
void CMemoryStream::Write(const void* p, unsigned long iLen)
{
  // check if we need a larger buffer
  if(strm_ulPosition + iLen >= strm_ulSize) {
    AllocateMoreMemory(Max<int>(iLen, 1024));
  }
  ...
  // update used counter
  strm_ulUsed = Max<unsigned long>(strm_ulPosition, strm_ulUsed);
}

void* CMemoryStream::Read(unsigned long iLen)
{
  // create a new buffer
...
void CMemoryStream::AllocateMoreMemory(int ctBytes)
{
  ASSERT(ctBytes > 0);

  // create new buffer and remember old one
  unsigned char* pubNewBuffer = new unsigned char[strm_ulSize + ctBytes];
  unsigned char* pubOldBuffer = strm_pubBuffer;
